...
<script>
    $(document).ready(function() {
        ...
        $('.detail-btn').click(function() {
            var rootPath = "${root}";
            selectedTicketId = $(this).data('order-id'); // 클릭한 버튼의 data-order-id 값을 저장

            $.ajax({
                url: rootPath + '/admin/ticketDetail?orderId=' + selectedTicketId, // orderId 사용
                type: 'GET',
                success: function(ticket) {
                    $('#TicketDetailModalLabel').text('티켓 상세 정보 - 티켓 ID: ' + ticket.orderId);
                    var modalBody = $('#TicketDetailModal .modal-body');
                    modalBody.html('<p>경기 날짜: ' + ticket.gameDate + '</p>' +
                                   '<p>시작 시간: ' + ticket.startTime + '</p>' +
                                   '<p>경기 타이틀: ' + ticket.ticketTitle + '</p>' +
                                   '<p>선택 좌석: ' + ticket.ticketSeat + '</p>' +
                                   '<p>총액: ' + ticket.totalPrice + '</p>' +
                                   '<p>주문자 이름: ' + ticket.ordererName + '</p>' +
                                   '<p>주문자 연락처: ' + ticket.cellphoneNumber + '</p>' +
                                   '<p>주문자 이메일: ' + ticket.email + '</p>' +
                                   '<p>주문 상태:' + ticket.orderStatus + '</p>');

                    $('#TicketDetailModal').modal('show'); // 모달 표시
                },
